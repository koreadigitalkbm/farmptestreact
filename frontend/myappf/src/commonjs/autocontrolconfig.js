...
module.exports = class AutoControlconfig {
  ...
  static deepcopy(mobj) {
    const copyobj = JSON.parse(JSON.stringify(mobj));

    return copyobj;
  }

  static CopyObj(origindes, copysrc) {
    for (let key in origindes) {
      if (typeof origindes[key] === "object") {
        AutoControlconfig.CopyObj(origindes[key], copysrc[key]);
      } else {
        origindes[key] = copysrc[key];
      }
    }
  }

  static cmpObject(orgobj, copyobj) {
    

    // console.log(JSON.stringify(orgobj));
    //console.log(JSON.stringify(copyobj));

    if (JSON.stringify(orgobj) === JSON.stringify(copyobj)) {
      return true;
    }
    return false;
  }

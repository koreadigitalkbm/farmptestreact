...
const JukeboxWatersupplyM1 = (props) => {
  ...
  const [manualactname, setmanualactname] = React.useState("selitem1");
  ...
  const inputchangeHandler = (event) => {
    ...
    {
      case "ontimesec":    setmanualontimesec(event.target.value);     break;
      case "DOnTime":
  ...
  function manualonoff(ontimesec) {
    const actindex = manualactname === "selitem1" ? 0 : 1;
      ...
      <Stack spacing={1}>
        <FormControl>
          ...
          <RadioGroup aria-labelledby="demo-controlled-radio-buttons-group" name="controlled-radio-buttons-group" value={manualactname} onChange={handleChangeAct}>
            <FormControlLabel value="selitem1" control={<Radio />} label="급수펌프" />
